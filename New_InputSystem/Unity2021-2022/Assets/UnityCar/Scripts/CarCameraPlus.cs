 ...
 * Version: Unity 21-22+ (New Input)
 * Edits By: DeathwatchGaming
 * License: MIT
 */

// New Input System version using serialized Input Action Asset
...
using UnityEngine.InputSystem; // New Input System package for InputActionAsset and InputAction
...
namespace UnityCar.Scripts // Namespace for organization of UnityCar scripts
{
    public class CarCameraPlus : MonoBehaviour // Main camera controller class inheriting from MonoBehaviour
    {
        ...
        [Header("Input")]
        [Tooltip("Input Action Asset containing all actions for this camera.")]
        // Serialized Input Action Asset for New Input System. Must contain "ToggleCameraMode" and "Look" actions.
        ...
        // Input Actions
        private InputAction toggleModeAction; // Action for toggling camera mode
        private InputAction mouseDeltaAction; // Action for mouse look delta
        ...
        private void Awake()
        {
            ...
            // Find actions from the asset
            if (inputActions != null)
            {
                toggleModeAction = inputActions.FindAction("ToggleCameraMode");
                mouseDeltaAction = inputActions.FindAction("Look");
            }
            else
            {
                Debug.LogWarning("Input Action Asset is not assigned on CarCameraPlus!");
            }
        }
        ...
        {
            // Disable input actions when the script is inactive
            if (toggleModeAction != null) 
            {
        ...
        private void Update()
        {
            ...
            if (isOrbitMode && mouseDeltaAction != null)
            {
